...
POSITIVE_PAGES=['https://ol.3dmgame.com/gl/293382.html','https://www.9game.cn/yyslskfsjwx/10759407.html','https://www.taptap.cn/moment/625374395434536254']
...
def request(url,data=None,timeout=20):
    ...
    with urllib.request.urlopen(urllib.request.Request(url,data=body,headers=headers),timeout=timeout) as r:
        return r.status,r.headers.get('Content-Type',''),r.read(8_000_000)
def fetch_json(url,data):
    st,ct,b=request(url,data); return {'status':st,'contentType':ct,'sha256':hashlib.sha256(b).hexdigest(),'bytes':len(b),'json':json.loads(b.decode('utf-8','replace'))}
def iter_dicts(v):
...
def annulus_mask(h,w):
    yy,xx=np.ogrid[:h,:w]; cx=(w-1)/2; cy=(h-1)/2; r=np.sqrt((xx-cx)**2+(yy-cy)**2)
    return ((r>=min(h,w)*.12)&(r<=min(h,w)*.44)).astype(np.uint8)*255
def js(a,b):
    a=np.asarray(a,np.float64); b=np.asarray(b,np.float64); a=a/max(1e-12,a.sum()); b=b/max(1e-12,b.sum()); m=.5*(a+b)
    def kl(x,y):
        q=x>0; return float(np.sum(x[q]*np.log2(x[q]/np.maximum(y[q],1e-12))))
    return .5*kl(a,m)+.5*kl(b,m)
def descriptor(img,mask=None):
    if img is None: return None
    img=cv2.resize(img,(192,192),interpolation=cv2.INTER_AREA); mask=mask if mask is not None else annulus_mask(192,192)
    ...
    valid=mask>0; threshold=float(np.percentile(mag[valid],72)); edge=(mag>=max(8,threshold))&valid
    oh=np.histogram(ang[edge],bins=12,range=(0,180))[0].astype(float); oh/=max(1,oh.sum())
    vals=gray[valid].astype(np.float32); hg=np.histogram(vals,bins=32,range=(0,256))[0].astype(float); p=hg/max(1,hg.sum()); p=p[p>0]; entropy=float(-(p*np.log2(p)).sum())/5
    f=np.fft.rfft2(gray.astype(np.float32)-float(gray.mean())); power=np.abs(f)**2; hh,ww=power.shape; yy,xx=np.ogrid[:hh,:ww]; rr=np.sqrt((yy/max(1,hh))**2+(xx/max(1,ww))**2)
    low=float(power[(rr>.02)&(rr<.16)].mean()); high=float(power[(rr>.22)&(rr<.55)].mean()); hf=math.log1p(high/max(1e-6,low))
    kp=cv2.ORB_create(nfeatures=800,fastThreshold=10).detect(gray,mask) or []
    scalars=np.array([float(hsv[:,:,1][valid].mean()/255),float(gray[valid].std()/128),float(edge.sum()/max(1,valid.sum())),entropy,hf,min(1,len(kp)/300)],np.float32)
    return {'vec':np.concatenate([h,s,g,oh,scalars]).astype(np.float32),'h':h,'s':s,'g':g,'orient':oh,'edgeDensity':float(scalars[2]),'entropy':entropy,'hfRatio':hf,'orbDensity':float(scalars[-1])}
def family_summary(rows,fixture_desc):
    vec=np.stack([r['vec'] for r in rows]); fvec=np.mean(np.stack([r['vec'] for r in fixture_desc]),axis=0); center=np.median(vec,axis=0); scale=np.maximum(np.median(np.abs(vec-center),axis=0)*1.4826,.025)
    d=np.sqrt(np.mean(((vec-center)/scale)**2,axis=1)); fd=float(np.sqrt(np.mean(((fvec-center)/scale)**2))); p90=float(np.quantile(d,.9)); ratio=fd/max(.1,p90)
...
def download_page_images(page,out,limit=60):
    rec={'page':page,'status':None,'images':[]}
    try:
        st,ct,b=request(page); rec['status']=st; p=ImgParser(); p.feed(b.decode('utf-8','replace')); seen=[]
        for u in p.urls:
            u=urllib.parse.urljoin(page,u)
            if u.startswith('data:') or u in seen: continue
            seen.append(u)
        page_key=hashlib.sha1(page.encode()).hexdigest()[:8]
        for u in seen[:limit]:
            try:
                ...
                dst=out/f'{page_key}-{len(rec["images"]):02d}.jpg'; cv2.imwrite(str(dst),im)
                rec['images'].append({'url':u,'path':str(dst),'bytes':len(data),'width':int(im.shape[1]),'height':int(im.shape[0])})
            except Exception: pass
    except Exception as e: rec['error']=f'{type(e).__name__}: {e}'
    return rec
def register_control(im,atlas,expected):
    scale=min(1.0,1600/max(im.shape[:2])); src=cv2.resize(im,None,fx=scale,fy=scale,interpolation=cv2.INTER_AREA) if scale<1 else im
    g1=cv2.cvtColor(src,cv2.COLOR_BGR2GRAY); g2=cv2.cvtColor(atlas,cv2.COLOR_BGR2GRAY); sift=cv2.SIFT_create(nfeatures=8000,contrastThreshold=.02,edgeThreshold=16)
    ...
    a=np.float32([k1[m.queryIdx].pt for m in good]).reshape(-1,1,2); b=np.float32([k2[m.trainIdx].pt for m in good]).reshape(-1,1,2)
    H,mask=cv2.findHomography(a,b,cv2.RANSAC,5.0,maxIters=6000,confidence=.999)
    if H is None or mask is None: return {'registered':False,'good':len(good),'reason':'homography-failed'}
    inliers=int(mask.sum()); ratio=inliers/max(1,len(good)); h,w=src.shape[:2]; corners=np.float32([[[0,0]],[[w,0]],[[w,h]],[[0,h]]]); poly=cv2.perspectiveTransform(corners,H).reshape(-1,2)
    inside=cv2.pointPolygonTest(poly.astype(np.float32),(float(expected[0]),float(expected[1])),False)>=0; center=cv2.perspectiveTransform(np.float32([[[w/2,h/2]]]),H)[0,0]
...
def discover_official_sources():
    ...
    try:
        st,ct,b=request(OFFICIAL_MAP); report['status']=st; text=b.decode('utf-8','replace'); scripts=[urllib.parse.urljoin(OFFICIAL_MAP,u) for u in re.findall(r'<script[^>]+src=["\']([^"\']+)',text,re.I)]; strings=set(re.findall(r'https?://[^"\'\s)<>]+',text))
        for u in scripts[:24]:
            row={'url':u}
            try:
                sct,sctype,sb=request(u,timeout=15); row.update({'status':sct,'contentType':sctype,'bytes':len(sb),'sha256':hashlib.sha256(sb).hexdigest()}); txt=sb.decode('utf-8','replace')
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--fixture',type=Path,required=True); ap.add_argument('--cache',type=Path,required=True); ap.add_argument('--expanded-invariant',type=Path,required=True); ap.add_argument('--report',type=Path,required=True); ap.add_argument('--work-dir',type=Path,required=True)
    args=ap.parse_args(); args.work_dir.mkdir(parents=True,exist_ok=True); args.report.parent.mkdir(parents=True,exist_ok=True)
    capture=json.loads((args.fixture/'capture.json').read_text(encoding='utf-8-sig')); frames=capture.get('frames',[])
    ...
    cfg=fetch_json(CFG_URL,{'id':CFG_ID}); mapinfo=fetch_json(MAP_INFO_URL,{'appKey':'h72'}); points=fetch_json(POINT_URL,{'mapId':MAP_ID,'subTypes':['1','2'],'mapZone':'map12'}); result=cfg['json'].get('result',{})
    map_cfg=parse_ext(item(result,'地图信息') or {},{}); subtype_map=parse_ext(item(result,'subType与一级区域映射') or {},{}); planes=item(result,'多位面素材配置') or {'itemList':[]}; plane_rows=[]
    ...
    unique={(x,y):name for x,y,name in poi}; cover={'subtype1UniqueCoordinatePoints':len(unique),'boundedFineCache':{'z':5,'minX':12,'maxX':28,'minY':9,'maxY':24},'regions':{}}
    xmin=1e9;xmax=-1e9;ymin=1e9;ymax=-1e9; cache_count=0
    ...
    official=discover_official_sources(); expanded=json.loads(args.expanded_invariant.read_text(encoding='utf-8')); ev={'candidateRecallVerdict':expanded.get('candidateRecallVerdict'),'exactRemainingFailureStage':expanded.get('exactRemainingFailureStage'),'productChangeJustified':expanded.get('productChangeJustified'),'best':None}
