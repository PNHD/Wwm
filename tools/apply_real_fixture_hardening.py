...
if "REAL_FIXTURE_V1_DIRECT" in source:
    print('vision-sync.js already hardened; no-op')
    raise SystemExit(0)
...
def one(old, new, label):
    ...
    if count != 1:
        raise RuntimeError(f'{label}: expected exactly 1 literal match, got {count}')
...
one("roiFraction:loadNumber(ROI_KEY,.2,.12,.34)", "roiFraction:loadNumber(ROI_KEY,.25,.12,.34)", 'default-roi-fraction')
regex_one(
    r"function roi\(video\)\{.*?\}\nfunction preview",
...
motion_feature_patch = r'''function visionPercentile(values,p){if(!values.length)return 0;const a=Array.from(values).sort((x,y)=>x-y);return a[Math.min(a.length-1,Math.max(0,Math.floor((a.length-1)*p)))]}
...
regex_one(r"function features\(\)\{.*?\}\nfunction correlation", motion_feature_patch, 'translucent-minimap-motion-normalization')

rank_old = "(b.rankScore??b.score)-(a.rankScore??a.score)"
rank_count = source.count(rank_old)
if rank_count < 4:
    raise RuntimeError(f'structural-rerank-only: expected >=4 matches, got {rank_count}')
source = source.replace(rank_old, "(b.rankScore??-Infinity)-(a.rankScore??-Infinity)")
applied.append(f'structural-rerank-only:{rank_count}')

one("size=Math.min(w,h)*.20,cx=w*.14,cy=h*.17", "size=Math.min(w,h)*.25,cx=Math.min(w,h)*.125,cy=Math.min(w,h)*.125", 'synthetic-capture-roi-geometry')
one("async function registerFrame(cfg,mapId){const roiCanvas=VISION.captureRoi(192);", "async function registerFrame(cfg,mapId,overrideCanvas=null){const roiCanvas=overrideCanvas||VISION.captureRoi(192);", 'register-frame-override-canvas')
one(
...
absolute_replay_patch = r'''window.__WWMSYNC_ABSOLUTE_REPLAY__={
...
one("window.__WWMSYNC_ABSOLUTE_DIAGNOSTICS__=", absolute_replay_patch, 'absolute-replay-hooks')

source += "\nwindow.__WWMSYNC_VISION_HARDENING__={version:'REAL_FIXTURE_V1',implementation:'REAL_FIXTURE_V1_DIRECT',normalization:'adaptive-translucent-map'};\n"
VISION.write_text(source, encoding='utf-8')

index = INDEX.read_text(encoding='utf-8')
if '/vision-sync-loader.js' not in index:
    raise RuntimeError('index.html does not reference the transitional loader')
index = index.replace('/vision-sync-loader.js', '/vision-sync.js')
INDEX.write_text(index, encoding='utf-8')

print('Applied:', ', '.join(applied))
print('Wrote direct CSP-safe hardening to vision-sync.js and restored direct script load.')
