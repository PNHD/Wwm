...
function sequenceScore(candidate, c) {
  const dir = Number.isFinite(c.meanDirectionCosine) ? (c.meanDirectionCosine + 1) / 2 : 0;
  const endpointFit = Math.exp(-c.endpointDisplacementError / Math.max(18, c.predictedEndDisplacement.magnitude || 18));
  const angleFit = Math.exp(-(c.angleStability.maxDeltaFromStartDeg ?? 180) / 25);
  const scaleFit = Math.exp(-(c.scaleStability.maxRelativeDeltaFromStart ?? 1) / .15);
  return clamp(.18 * (candidate.generatorScore || 0) + .26 * dir + .22 * endpointFit + .16 * angleFit + .12 * scaleFit + .06 * clamp((candidate.inlierCount || 0) / 12, 0, 1), 0, 1);
}
...
const candidates = (generated.candidates || []).slice(0, TOP_K);
fs.mkdirSync(path.dirname(reportPath), { recursive: true });
if (!candidates.length) {
  const report = {
    schema: 'wwmsync-real-gfn-local-feature-hypotheses-sequence-v1', generatedAtUtc: new Date().toISOString(),
    fixture: { frameCount: frames.length, fps: capture.fps, intervalMs: capture.intervalMs },
    scope: { replayOnly: true, productRuntimeChanged: false, productionGateLowered: false, knownFalseCandidateOptimized: false },
    candidateGenerator: { schema: generated.schema, candidateCount: 0, methods: generated.methods, representationPairs: generated.representationPairs },
    sequenceRankedTopK: [], stableCandidateCount: 0, bestStableCandidate: null,
    hardNegativeSeparation: { distinctNegativeCount: 0, referenceSequenceScore: null, maxDistinctNegativeSequenceScore: null, positiveSeparation: null },
    candidateRecallVerdict: 'NO-TRUE-LIKE-CANDIDATE', exactRemainingFailureStage: 'local-feature-hypothesis-candidate-generation',
    productChangeJustified: false,
    productChangeRecommendation: 'Do not modify production runtime. Classical local-feature correspondence produced no global hypotheses; escalate representation to a learned local feature matcher/retriever.',
    realGfnFixtureAcceptance: { status: 'FAIL', reason: 'No local-feature global hypotheses were generated.' }, liveTestingJustified: false,
  };
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  console.log(JSON.stringify({ report: reportPath, candidateRecallVerdict: report.candidateRecallVerdict, exactRemainingFailureStage: report.exactRemainingFailureStage }, null, 2));
  process.exit(0);
}
...
try {
  ...
  page.on('pageerror', e => console.error('[feature hypotheses pageerror]', e.message));
  page.on('console', m => { if (m.type() === 'error') console.error('[feature hypotheses console]', m.text()); });
  ...
  for (const candidate of candidates) {
    let seed = { x: candidate.x, y: candidate.y, radius: candidate.radius, angle: candidate.angle }, sequence = [], key = `feature-${candidate.rank}`;
    for (let i = 0; i < 40; i++) {
      ...
      }, { name: frames[i].minimap, seed, key });
      if (!tracked) { sequence = []; break; }
    ...
    if (sequence.length !== 40) {
      hypotheses.push({ candidate, sequenceScore: 0, consistency: { samePhysicalBasinStable: false, reason: `tracking-failed-after-${sequence.length}-frames` }, basinAnchor: { x: candidate.x, y: candidate.y, radius: candidate.radius, angle: candidate.angle }, trackedSequence: sequence });
      continue;
    }
  ...
  const stable = hypotheses.filter(h => h.consistency?.samePhysicalBasinStable);
  const bestStable = stable[0] || null;
  const reference = bestStable || hypotheses[0];
  const distinctNegatives = hypotheses.filter(h => h !== reference && (hypot(h.basinAnchor.x - reference.basinAnchor.x, h.basinAnchor.y - reference.basinAnchor.y) > 120 || angleDelta(h.basinAnchor.angle, reference.basinAnchor.angle) > 25 || Math.abs(Math.log((h.basinAnchor.radius || 1) / (reference.basinAnchor.radius || 1))) > .20));
  const maxNegative = distinctNegatives.length ? Math.max(...distinctNegatives.map(h => h.sequenceScore)) : null;
  const separation = Number.isFinite(maxNegative) ? reference.sequenceScore - maxNegative : null;
  const trueLike = !!bestStable && bestStable.sequenceScore >= .55 && (separation == null || separation > .005);
  const report = {
    schema: 'wwmsync-real-gfn-local-feature-hypotheses-sequence-v1', generatedAtUtc: new Date().toISOString(),
    fixture: { frameCount: frames.length, fps: capture.fps, intervalMs: capture.intervalMs },
    scope: { replayOnly: true, productRuntimeChanged: false, productionGateLowered: false, knownFalseCandidateOptimized: false },
    candidateGenerator: { schema: generated.schema, candidateCount: generated.candidates?.length || 0, methods: generated.methods, representationPairs: generated.representationPairs },
    sequenceRankedTopK: hypotheses,
    stableCandidateCount: stable.length,
    bestStableCandidate: bestStable,
    hardNegativeSeparation: { distinctNegativeCount: distinctNegatives.length, referenceSequenceScore: reference?.sequenceScore ?? null, maxDistinctNegativeSequenceScore: maxNegative, positiveSeparation: separation },
    candidateRecallVerdict: trueLike ? 'TRUE-LIKE-CANDIDATE-SURFACED' : 'NO-TRUE-LIKE-CANDIDATE',
    exactRemainingFailureStage: trueLike ? 'production-integration-not-applied-diagnostic-only' : 'local-feature-hypothesis-candidate-recall',
    productChangeJustified: trueLike,
    productChangeRecommendation: trueLike ? 'Local invariant correspondence surfaced a sequence-consistent basin. Next step is to reproduce the generator deterministically in a production-suitable implementation while keeping the existing fine matcher/gates unchanged.' : 'Do not modify production runtime. Classical local-feature correspondence did not surface a sequence-consistent TRUE-LIKE basin on the exact real GFN fixture; escalate representation to a learned local feature matcher/retriever rather than tuning the rejected structural candidates.',
    realGfnFixtureAcceptance: { status: 'FAIL', reason: trueLike ? 'Candidate recall diagnostic succeeded but no production absolute lock/AUTO matrix/periodic correction was applied.' : 'No TRUE-LIKE candidate surfaced from local-feature hypotheses.' },
    liveTestingJustified: false,
  };
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  console.log(JSON.stringify({ report: reportPath, candidateRecallVerdict: report.candidateRecallVerdict, stableCandidateCount: stable.length, bestStable: bestStable ? { rank: bestStable.candidate.rank, x: bestStable.candidate.x, y: bestStable.candidate.y, radius: bestStable.candidate.radius, angle: bestStable.candidate.angle, generatorScore: bestStable.candidate.generatorScore, sequenceScore: bestStable.sequenceScore, directionCosine: bestStable.consistency.meanDirectionCosine, endpointErrorRatio: bestStable.consistency.endpointErrorRatio, angleMaxDelta: bestStable.consistency.angleStability.maxDeltaFromStartDeg, scaleMaxRelativeDelta: bestStable.consistency.scaleStability.maxRelativeDeltaFromStart } : null, separation, productChangeJustified: trueLike }, null, 2));
} finally {
